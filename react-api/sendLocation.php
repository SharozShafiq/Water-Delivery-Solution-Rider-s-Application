...
elseif (!isset($data->rider_id)) {
    ...
    $fields = ['fields' => ['rider_id','lats','longs']];
...
// IF THERE ARE NO EMPTY FIELDS THEN-
    ...
    $lats = trim($data->lats);
    $longs = trim($data->longs);
    
    if(isset($data->reride)) {
        $delete_query = "DELETE location WHERE rider_id='".$rider_id."' AND '".$order_id."'";

                $delete_stmt = $conn->query($insert_query);
    }
        ...
        try {
            ...
            if($query_stmt->num_rows > 0){
                ...
                $insert_query = "INSERT INTO `location`(`rider_id`,`order_id`,`lats`,`longs`) VALUES('".$rider_id."','".$order_id."','".$lats."','".$longs."')";
